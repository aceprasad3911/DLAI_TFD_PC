...
model = tf.keras.models.Sequential([
    ...
    tf.keras.layers.Dense(3, activation='softmax') # Activation changed from sigmoid to softmax to give a
                                                         # probability of each option with highest x/1 being the
                                                         # most likely option
])
...
train_dataset = tf.keras.utils.image_dataset_from_directory(
    ...
    image_size=(150, 150),
    batch_size=32,
    label_mode='categorical' # Class mode changed from binary to categorical to handle multiple categories
    )
...
model_with_aug.compile(loss = 'categorical_crossentropy', optimizer='rmsprop', metrics=['accuracy'])
                            # binary crossentropy changed to categorical crossentropy
# Train the model
